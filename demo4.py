...
def run_model(model, rgbs, N, sw):
    ...
    print('N_ IS CALCULATING AS ', N_)
    grid_y, grid_x = utils.basic.meshgrid2d(B, N_, N_, stack=False, norm=False, device='cuda')
    grid_y = 100 + grid_y.reshape(B, -1) / float(N_ - 1) * (H/3 - 16)
    grid_x = 100 + grid_x.reshape(B, -1) / float(N_ - 1) * (W/2 - 16)
 ...
 #  print_stats('trajs_e', trajs_e)
    ...
    if sw is not None and sw.save_this:
     ...
     #  print('Inputting tensor with size ', trajs_e.size(),' into sw.summ.traj')
     #  print('Input is', trajs_e[0:1])
        ...
        wide_sing = torch.cat([o2])
        sw.summ_rgbs('outputs/wide_cat', wide_cat.unbind(1))

        # write to disk, in case that's more convenient
        wide_list = list(wide_sing.unbind(1))
        wide_list = [wide[0].permute(1, 2, 0).cpu().numpy() for wide in wide_list]
        wide_list = [Image.fromarray(wide) for wide in wide_list]
        out_fn = './out_%d.gif' % sw.global_step
        os.makedirs('gifs_demo', exist_ok=True)
        wide_list[0].save(os.path.join('gifs_demo', out_fn), save_all=True, append_images=wide_list[1:])
...
def main():
    ...
    print('LENGTH OF FILE NAMES IS', len(filenames))
